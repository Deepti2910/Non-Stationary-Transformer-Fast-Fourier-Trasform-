...
class Exp_Main(Exp_Basic):
    ...
    # Visualize function (used in test)
    def visual(gt, pd, path):
    ...
    def test(self, setting, test=0):
        ...
        # Reshape for metric computation and visualization
        preds = preds.reshape(-1, preds.shape[-2], preds.shape[-1])
        trues = trues.reshape(-1, trues.shape[-2], trues.shape[-1])
        print('Reshaped test shape:', preds.shape, trues.shape)

        # Plotting full predicted vs true values (only for first feature)
        y_pred_all = preds[:, :, 0].flatten()
        y_true_all = trues[:, :, 0].flatten()
        visual(y_true_all, y_pred_all, os.path.join(result_folder, 'final_plot.pdf'))
    ...
    #             ...
    #         print("Prediction shape:", np.array(preds).shape)
    #         print("Testing done.")
    #         return
    #
    #     test_data, test_loader = self._get_data(flag='test')
    ...
    def predict(self, setting, load=False):
        ...
        return
# import pandas as pd
# df = pd.read_csv('C:/Users/deept/PycharmProjects/Nonstationary_Transformers/dataset/ETT-small/ETTh1.csv')
